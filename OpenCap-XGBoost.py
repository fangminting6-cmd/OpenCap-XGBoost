...
ADVICE_MAP = {
    "HFA": (
        ...
        "**训练建议：** \n"
        "1. **基础 (髋铰链重塑)：** 罗马尼亚硬拉 (RDL)。\n   *细节*：强调臀部向后推，膝盖微屈但不前推，感受大腿后侧和臀部的强烈拉伸感。保持背部平直。*(3组 x 8-12次)* \n"
        "2. **进阶 (髋部爆发力)：** 壶铃摆动 (Kettlebell Swing)。\n   *细节*：利用臀部爆发式伸展“弹”起壶铃，而非手臂发力。在动作顶点主动收紧臀肌和核心。*(3组 x 15次)* \n"
        "3. **动态 (落地力学)：** 高台深落地 (Drop Landing)。\n   *细节*：从 30-40cm 高台迈下（不要向上跳），双脚触地瞬间迅速“向后坐”，像坐椅子一样，胸部保持挺直。*(3组 x 5-8次)* \n\n"
    ...
    "HAA": (
        ...
        "**训练建议：** \n"
        "1. **激活 (目标肌肉)：** 弹力带蚌式开合 / 抗阻侧抬腿。\n   *细节*：将弹力带绑在膝盖上方，动作过程中保持骨盆绝对稳定，不要向后翻转，体会臀部侧上方（臀中肌）的酸胀感。*(3组 x 15-20次/侧)* \n"
        "2. **力量 (动态抗内扣)：** 怪兽步 (Monster Walk)。\n   *细节*：保持 1/4 蹲姿，双脚略宽于肩，侧向或斜向行走时，主动对抗弹力带拉力，绝不允许膝盖内扣。*(3组 x 15米/方向)* \n"
        "3. **功能性 (单腿控制)：** 单腿下蹲 (Single-leg Box Squat)。\n   *细节*：对着镜子练习，下蹲至后方椅子上再站起。全程盯住膝盖，确保髌骨中心始终对准第2-3足趾。*(3组 x 8-10次/侧)* \n\n"
    ...
    "KFA": (
        ...
        "**训练建议：** \n"
        "1. **基础 (等长收缩)：** 靠墙静蹲。\n   *细节*：大腿与地面平行（或略高），重心压在足中和脚后跟，下背部贴紧墙壁，感受大腿前侧的持续发力。*(3-4组 x 45-60秒)* \n"
        "2. **离心 (制动能力)：** 诺迪克后屈腿 (Nordic Curls)。\n   *细节*：固定双脚，身体保持挺直缓慢向前倾倒。尽最大努力用大腿后侧（腘绳肌）控制下降速度，最后用手撑地。*(3组 x 5-8次)* \n"
        "3. **专项 (吸震反应)：** 连续跳深缓冲 (Altitude Drop)。\n   *细节*：从高处跳下，触地瞬间主动屈膝屈髋来“吸收”冲击力，要求落地声音极轻（像猫一样），定格 2 秒检查姿势。*(3组 x 6-8次)* \n\n"
    ...
    "ADF": (
        ...
        "**训练建议：** \n"
        "1. **松解 (软组织恢复)：** 泡沫轴/筋膜球深度松解。\n   *细节*：重点滚压小腿肚（腓肠肌）和靠近跟腱的区域（比目鱼肌），遇到痛点停留 20 秒并配合脚踝转动。*(双侧各 1-2 分钟)* \n"
        "2. **拉伸 (被动活动度)：** 靠墙弓箭步背屈拉伸 (Knee-to-Wall)。\n   *细节*：脚尖朝向墙壁，足跟死死踩住地面不离开，膝盖尽量向前顶触碰墙壁。记录脚尖离墙的最远距离以评估进步。*(3组 x 每侧保持 30-45秒)* \n"
        "3. **活动度 (动态受压)：** 负重高脚杯深蹲停留。\n   *细节*：抱重物进入最深蹲姿，将重物压在单侧大腿前侧，迫使膝盖向前超过脚尖，同时保持脚后跟踩实。*(3组 x 每侧压迫 20秒)* \n\n"
    ...
    "FPA": (
        ...
        "**训练建议：** \n"
        "1. **平衡 (足底感知)：** 赤足单腿闭眼站立。\n   *细节*：脱鞋，感受前脚掌内侧、前脚掌外侧和足跟构成的“足底三角”均匀受力，脚趾轻轻抓地（短足发力）。*(3组 x 30-45秒/侧)* \n"
        "2. **整合 (力线纠正)：** 弓箭步走 (Lunge Walk)。\n   *细节*：不仅要求步幅达标，更要强制盯住前脚和后脚，必须100%朝向正前方，拒绝脚尖外撇代偿。*(3组 x 每侧 10-12步)* \n"
        "3. **敏捷 (动态对齐)：** “十字象限跳” (Dot Drill)。\n   *细节*：在地上标记十字，双脚并拢在四个象限间快速跳跃。关注落地时双脚的平行度，避免在疲劳和变向时出现外八。*(3组 x 20-30秒)* \n\n"
    ...
    "TFA": (
        ...
        "**训练建议：** \n"
        "1. **静态 (侧向抗屈曲)：** 侧桥 (Side Plank)。\n   *细节*：手肘与肩膀垂直，从头到脚跟呈一条直线，主动将骨盆顶起，不要塌腰，核心绷紧。*(3组 x 30-45秒/侧)* \n"
        "2. **动态 (抗侧屈行走)：** 单臂农夫行走 (Suitcase Carry)。\n   *细节*：单手提大重量哑铃或壶铃行走。刻意对抗重力，保持双肩水平、躯干绝对竖直，不向负重侧倾斜。*(3组 x 每侧 20米)* \n"
        "3. **专项 (动态中立)：** 滑冰跳 / 抗阻侧向跳 (Skater Jumps)。\n   *细节*：左右单腿横向跳跃。落地时要求“粘在地上”停顿 2 秒，控制上半身不要随着惯性过度侧倾，保持鼻子在脚尖正上方。*(3组 x 每侧 8-10次)* \n\n"
